...
```{r, fig.width = 12, fig.height = 6, warning = TRUE, message = FALSE}
b1_stack <- barplot_gg_stack(as.data.frame(as.table(Surv_Class)[2:1, ])) + 
  theme_bw(base_family = "KoPubWorldDotum Light") +
  labs(x = "객실 등급", y = "인원(명)") +
...
```{r, fig.width = 7.2, fig.height = 6, warning = TRUE, message = FALSE}
m1_list <- 
  mosaic_gg_new(as.data.frame(as.table(Surv_Class)[2:1, ]))
m1_list$m +
  theme_bw(base_family = "KoPubWorldDotum Light") +
...
```{r, fig.width = 12, fig.height = 6, warning = TRUE, message = TRUE}
...
```{r, fig.width = 7.2, fig.height = 6, warning = TRUE, message = FALSE}
m2_list <- 
  mosaic_gg_new(as.data.frame(as.table(Surv_Sex)[2:1, ]))
m2_list$m +
  theme_bw(base_family = "KoPubWorldDotum Light") +
  labs(x = "남녀별 누적(%)", y = "백분율(%)") + 
  ggtitle("남녀별 생존/사망") + 
  scale_fill_manual(name = "", 
                    labels = c("생존", "사망"), 
                    values = rainbow(2)[2:1]) +
  theme(plot.title = element_text(hjust = 0.5, family = "KoPubWorldDotum Bold"))
...
```{r, fig.width = 7.2, fig.height = 6, warning = TRUE, message = FALSE}
m3_list <- 
  mosaic_gg_new(as.data.frame(as.table(Surv_Age)[2:1, ]))
m3_list$m +
  theme_bw(base_family = "KoPubWorldDotum Light") +
...
```{r, fig.width = 7.2, fig.height = 6, warning = TRUE, message = FALSE}
m4_list <- 
  mosaic_gg_new(as.data.frame(as.table(Female_Class)[2:1, ]))
m4_list$m +
  theme_bw(base_family = "KoPubWorldDotum Light") +
  labs(x = "객실 등급 누적(%)", y = "백분율(%)") + 
...
m5_list <- 
  mosaic_gg_new(as.data.frame(as.table(Child_Class)[2:1, -4]))
m5_list$m +
  theme_bw(base_family = "KoPubWorldDotum Light") +
  labs(x = "객실 등급 누적(%)", y = "백분율(%)") + 
