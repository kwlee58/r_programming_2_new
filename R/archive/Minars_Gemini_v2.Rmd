...
# --- FIX: 중요한 도시 필터링 목록 수정 (Moskva -> Moskwa, Berezina -> Beresina) ---
# Minard.cities 데이터셋에서 사용되는 정확한 이름 사용
key_cities <- c("Kowno", "Wilna", "Witebsk", "Smolensk", "Moskwa", "Beresina", "Mojaisk")

cities_df_filtered <- cities_df %>%
    filter(city %in% key_cities)
```

# 2) 날짜 기반 경로 애니메이션 (온도 색상 반영)

```{r animate-path-with-temp, message = FALSE, include = TRUE, out.width="100%"}
# Minard 지도의 색상 설정 (진군: 황토색/붉은색, 후퇴: 온도에 따른 색상)
minard_colors <- c(
  "Advance" = "#E1B08C", # 진군: 황토색
  "Retreat" = "#3B5998"  # 후퇴: 진한 푸른색 (온도 스케일이 덮어쓸 예정)
)

p_path_temp <- ggplot(troops_merged, aes(long, lat, group = group)) +
  # 경로 그리기: 사이즈는 생존자(survivors), 색상은 온도(temp) 또는 진군 색상
  geom_path(aes(size = survivors, color = color_value, group = group), 
            lineend = "round") +
  
  # 온도 스케일 (후퇴 경로의 색상을 결정)
  scale_color_gradient(
    low = "#0000FF", # 매우 추울 때 (진한 파랑)
    high = "#FF0000", # 덜 추울 때 (붉은색. 다만 온도가 낮으므로 진하게 나오진 않음)
    limits = c(-30, 10), # 미나르 지도의 온도 범위
    oob = scales::squish,
    name = "Temperature (°C)",
    guide = "none" # 색상 범례는 별도 설명으로 대체
  ) +
  
  scale_size(range = c(0.5, 5), guide = "none") +
  coord_quickmap() +
  
  # --- FIX: 필터링된 도시만 표시하고, 라벨 위치와 크기를 조정 ---
  geom_label(data = cities_df_filtered, aes(long, lat, label = city),
             inherit.aes = FALSE, vjust = 1.5, size = 4, 
             fill = "white", label.size = 0.1, color = "black") +
...
gif_path_temp <- animate(anim_path_temp,
                         nframes = 150, fps = 10, 
                         width = 1000, height = 650, # Height increased
                         units = "px",
                         bg = "white",
...
p_temp_final <- ggplot(temps, aes(long, temp)) +
  ...
  geom_text(aes(label = temp_lbl, group = temp_id), vjust = -1.5, na.rm = TRUE, size = 4) +
