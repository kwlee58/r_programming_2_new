...
doorae$"성과_사업착수" %<>%
  factor(levels = c("Yes", "No"), 
         labels = c("착수", "미착수"))
#> with()
  bi_corp     <- with(doorae, which(`공동체상태` == "경영개선" & `조직형태` == "법인"))
  bi_noncorp  <- with(doorae, which(`공동체상태` == "경영개선" & `조직형태` == "임의단체"))
  bp_corp     <- with(doorae, which(`공동체상태` == "창업준비" & `조직형태` == "법인"))
  bp_noncorp  <- with(doorae, which(`공동체상태` == "창업준비" & `조직형태` == "임의단체"))
