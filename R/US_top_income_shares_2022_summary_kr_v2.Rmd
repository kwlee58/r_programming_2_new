...
if (Sys.info()[["sysname"]] == "Windows") {
  win_home <- Sys.getenv("USERPROFILE")  # VMware Windows에서도 정상 동작
...
# 10% 단위
data_10_df <- 
  US_top_income_shares_22 %>%
  select(Year, P90_100)

data_10_tbl <- 
  US_top_income_shares_22 %>%
  select(Year, P99_100, P95_99, P90_95) %>%
  pivot_longer(
    cols = c(P99_100, P95_99, P90_95),
...
data_1_tbl <- 
  US_top_income_shares_22 %>%
  select(Year, P99.9_100, P99.5_99.9, P99_99.5) %>%
  pivot_longer(
    cols = c(P99.9_100, P99.5_99.9, P99_99.5),
    names_to = "Percentiles",
    values_to = "Share"
  ) %>%
  mutate(Percentiles = factor(Percentiles,
...
data_01_tbl <- 
  US_top_income_shares_22 %>%
  select(Year, P99.99_100, P99.9_99.99) %>%
  pivot_longer(
    cols = c(P99.99_100, P99.9_99.99),
    names_to = "Percentiles",
    values_to = "Share"
  ) %>%
  mutate(Percentiles = factor(Percentiles,
...
```{r, tidy, echo = FALSE, message = FALSE, eval = FALSE}
library(tidyverse)
data_10_df <- 
...
data_10_tbl <- 
  ...
  as_tibble %>%
  pivot_longer(
    cols = -Year,          # Year를 제외한 모든 열을 긴 형식으로 변환 (gather의 -Year)
    names_to = "Percentiles", # 변환된 열 이름(P99_100 등)을 저장할 새 열 이름 (gather의 key)
    values_to = "Share"    # 변환된 값(숫자)을 저장할 새 열 이름 (gather의 value)
  ) %>%
...
ggplot(data = data_01_tbl, 
  ...
  theme(plot.title = element_text(size = 20, 
                                  hjust = 0.5)) +
  theme(legend.position = "inside", legend.position.inside = c(0.5, 0.88)) +
  scale_colour_manual(name = "", 
                      values = c("black", "blue"), 
...
미국 소득 불평등의 역사 ...top_income_shares_2022_summary_
