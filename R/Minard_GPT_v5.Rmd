...
temps <- temps %>% mutate(lbl = as.character(paste0(temp, "°C")))
...
```{r animate-temp, include=FALSE}
p_temp <- ggplot(temps, aes(long, temp)) +
  geom_line() + geom_point() +
  geom_text(aes(label = lbl), vjust = -0.7) +  # 문자형 라벨 사용
  theme_minimal() +
