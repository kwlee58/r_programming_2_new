...
literacy_data <- 
  read_excel("../data/literacy_class_version.xlsx",
             range = "A1:AA1004",
             col_names = TRUE)
...
#> class_roll 에 있는 group 을 id 에 맞추어(match) literacy_roll,  에 복제
literacy_data$group <- class_roll$group[match(literacy_data$id, class_roll$id)]

#> literacy_roll 에 그룹을 랜덤하게 추가 (예시이나 재현하려면 set.seed 사용하여야 함)
# set.seed(2)
...
```{r, echo = FALSE, fig.width = 9, fig.height = 12, message = FALSE}
...
score_type
# score_type <- 
#   aggregate(t(marks), list("유형" = type_kr), 
#             FUN = sum, na.rm = TRUE)
score_type == 0
