...
library(xlsx)
options(width = 180)
class_roll <- "../data/class_roll2002.xlsx" %>% 
  read.xlsx(sheetIndex = 1,
            startRow = 1, endRow = 84,
            colIndex = 2:3,
            header = TRUE,
            encoding = "UTF-8",
...
```{r}
BFI <- "../data/BFI2002.xlsx" %>% 
  read.xlsx(sheetIndex = 1, 
            startRow = 1, endRow = 60, 
            colIndex = 1:26, 
            header = TRUE, 
            encoding = "UTF-8",
            stringsAsFactors = FALSE)
...
```{r, save.image}
saveRDS(BFI, file = "./BFI2002.RDS")
saveRDS(MI, file = "./MI2002.RDS")
save.image("./BFI_MI_AR2002.RData")
```
