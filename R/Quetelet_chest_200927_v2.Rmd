...
```{r gof-two}
## ── 필수 계산 1 : gof_two ───────────────────────────────────
## 위 normal_gof() 를 두 판본에 각각 적용하고 rbind 로 2행으로 묶습니다.

gof_two <- rbind(quetelet = normal_gof(quetelet),
                 stigler  = normal_gof(stigler))
gof_two

## ── 필수 계산 2 : 결론이 갈리는가 ───────────────────────────

round(gof_two[, "p_value"], 4)
## 케틀레 0.0223 → 5%에서 기각.  스티글러 0.1428 → 기각 못 함.
## 전사 오류 여섯 명이 결론을 뒤집습니다.
```

```{r try-it-w3}
## ── 바꿔 보기 ───────────────────────────────────────────────
## 아래 줄의  0.05  를  0.01  로 바꾸고 다시 Knit 해 보세요.
## 두 판본 모두 "기각 못 함" 이 됩니다. 자료는 그대로인데 결론이 바뀝니다.
## 0.05 라는 기준선이 어디서 왔는지, 그것을 누가 정했는지가
## Comments 4번(한계)의 재료입니다.

gof_two[, "p_value"] < 0.05
```
